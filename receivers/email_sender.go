	...
	for _, msg := range messages {
		if msg.SingleEmail {
			...
			if err := gomail.Send(sender, m); err != nil {
				errs = errors.Join(errs, fmt.Errorf("failed to send notification to email address: %s: %w", strings.Join(msg.To, ";"), err))
			} else {
				sentEmailsCount++
			}
		} else {
			for _, recipient := range msg.To {
				...
				if err := gomail.Send(sender, m); err != nil {
					errs = errors.Join(errs, fmt.Errorf("failed to send notification to email address: %s: %w", recipient, err))
				} else {
					sentEmailsCount++
				}
			}
